...
//在分区part上打开inode结点为inode_no的目录并返回目录指针
...
//在part分区的pdir目录内寻找名为name的文件或目录
//找到后返回true并将其目录项存入dir_e中，否则返回false
bool search_dir_entry(struct partition* part, struct dir* pdir, \
					  const char* name, struct dir_entry* dir_e) {
    uint32_t block_cnt = 140; 		// 12个直接块+128个一级间接块

	//12个直接块大小+128间接块，共560字节
	uint32_t* all_blocks = (uint32_t* )sys_malloc(48 + 512);
	if (all_blocks == NULL) {
		printk("serch_dir_entry: sys_malloc for all_blocks failed\n");
		return false;
	}
    ...
    if(pdir->inode->i_sectors[12] != 0) {	//含有yiji间接块表
        ide_read(part->my_disk, pdir->inode->i_sectors[12], all_blocks + 12, 1);
    }
    //至此all_block存储的是该文件或目录的所有扇区地址

    //写目录项的时候已保证目录项不跨扇区
    //这样读目录项时容易处理，只申请容纳1扇区的内存
    uint8_t* buf = (uint8_t* )sys_malloc(SECTOR_SIZE);
    struct dir_entry* p_de = (struct dir_entry* )buf;		//p_de为指向目录项的指针，值为buf的起始地址
    uint32_t dir_entry_size = part->sb->dir_entry_size;
    uint32_t dir_entry_cnt = SECTOR_SIZE / dir_entry_size;	//一个扇区可容纳的目录项个数

    //开始在所有块中查找目录项
    while(block_idx < block_cnt) {
        //块地址为0时表示该块中无数据，继续在其他块中寻找
...
bool sync_dir_entry(struct dir* parent_dir, struct dir_entry* p_de, void* io_buf) {
    ...
    //将该目录的所有扇区地址(12个直接块+128个一级间接块)读到all_blocks中
    uint8_t block_idx = 0;
    ...
    //开始便利所有块以寻找目录项空位，若已有扇区中没有空闲位
    //在不超过文件大小的情况下，申请新扇区来存储新目录项
    block_idx = 0;
    while(block_idx < 140) {		//文件(包括目录)最大支持12个直接块+128个一级间接块
        block_bitmap_idx = -1;
        if( all_blocks[block_idx] == 0) {	//在三种情况下分配块
            ...
            else if(block_idx == 12) {			//若是尚未分配一级间接块表(block_idx==12表示第0个间接块地址为0)
				dir_inode->i_sectors[12] = block_lba;		//将上面分配的块做为一级间接块表地址
                block_lba = -1;
                block_lba = block_bitmap_alloc(cur_part);	//在分配一个块做为第0个间接块
                if(block_lba == -1) {
...
bool delete_dir_entry(struct partition* part, struct dir* pdir, uint32_t inode_no, void* io_buf) {
    ...
    while(block_idx < 140) {
            ...
            if((dir_e + dir_entry_idx)->f_type != FT_UNKNOWN) {
                if(!strcmp((dir_e + dir_entry_idx)->filename,".")) {
                    ...
                    if((dir_e + dir_entry_idx)->i_no == inode_no) {    //如果找到此i结点，就将其记录在dir_entry_found中
                        ASSERT(dir_entry_found == NULL);        //确保目录中只有一个编号为indeo_no的inode,找到一次后就不在是NULL
        ...
        if(dir_entry_cnt == 1 && !is_dir_first_block) {
            ...
            //b.将块地址从i_sectors或索引表中去掉
            if(block_idx < 12) {
                dir_inode->i_sectors[block_idx] = 0;
            }
            else {        //在一级间接索引表中擦除该间接块地址
                //先判断一级间接索引表中的数量，若只有这一个间接块，连同间接索引表所在的块一块回收
                ...
                if(indirect_blocks > 1) {            //间接索引表中还有其他间接块，则将当前间接块回收
                    all_blocks[block_idx] = 0;
                    ide_write(part->my_disk, dir_inode->i_sectors[12], all_blocks + 12, 1);
                }
                else {        //间接索引表中只有这一个间接块，连同间接索引表所在的块一块回收
                    //获授间接索引所在块
    ...
    // 所有块中没有找到则则返回false,若出现该情况应该是search_file出错了
    return false;
}
...
struct dir_entry* dir_read(struct dir* dir) {
    ...
    if(dir_inode->i_sectors[12] != 0) {        //含有一级间接块表
    ...
    uint32_t dir_entry_size = cur_part->sb->dir_entry_size;
    uint32_t dir_entrys_per_sec = SECTOR_SIZE / dir_entry_size;        //1扇区可以容纳的目录项数
    //因为此目录项内可能删除了某些文件或者子目录，所以要遍历所有块
    while(block_idx < block_cnt) {
        ...
        //遍历扇区内所有目录项
        while(dir_entry_idx < dir_entrys_per_sec) {
            if((dir_e + dir_entry_idx)->f_type) {        //如果f_type不为0,即不等于FT_UNKNOWN,
                //判断时不时最新的目录项，避免返回已经返回过的目录项
