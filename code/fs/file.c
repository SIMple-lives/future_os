...
int32_t pcb_fd_install(int32_t global_fd_idx) {
    ...
    uint8_t local_fd_idx = 3;   //从3开始，0，1，2为标准输入输出
    while(local_fd_idx < MAX_FILES_OPEN_PER_PROC) {
        if(cur->fd_table[local_fd_idx]==-1){    //找到空闲的文件描述符
...
int32_t block_bitmap_alloc(struct partition* part) {
    ...
    //和inode_bitmap_malloc不同，此处返回不是位图索引，而是具体可用的扇区地址
    return (part->sb->data_start_lba + bit_idx);
}

//将内存中bitmap第bit_idx位所在的512字节同步到硬盘
void bitmap_sync(struct partition* part, uint32_t bit_idx, uint8_t btmp_type) {
    uint32_t off_sec = bit_idx / 4096;        //本inode结点索引相当于位图扇区偏移量
    uint32_t off_size = off_sec * BLOCK_SIZE;    //本inode结点索引相当于位图字节的偏移量
    uint32_t sec_lba;
    uint8_t* bitmap_off;
    ...
    switch(btmp_type) {
        ...
        case BLOCK_BITMAP:
            ...
            break;
        default:
            PANIC("bitmap_sync: bitmap type error\n");
    }
...
int32_t file_create(struct dir* parent_dir, char* filename, uint8_t flag) {
    ...
    if (fd_idx == -1) {
        printk("file_create: no free file descripor\n");
        rollback_step = 2;
        goto rollback;
    }
    ...
    file_table[fd_idx].fd_inode->write_deny = false; //刚创建的文件没有被占用

    struct dir_entry new_dir_entry;
    memset(&new_dir_entry, 0, sizeof(struct dir_entry));

    create_dir_entry(filename, inode_no, FT_REGULAR, &new_dir_entry);//内存操作，不会返回失败

    //同步内存数据到硬盘
    //a在目录parent_dir下安装目录项new_dir_entry,写入硬盘后返回true,否则返回false
    if(!sync_dir_entry(parent_dir, &new_dir_entry, io_buf)) {
        printk("file_create: sync_dir_entry fail\n");
        rollback_step = 3;
        goto rollback;
    }

    memset(io_buf, 0, 1024);
    //b将父目录inode结点的内容同步到硬盘
    inode_sync(cur_part, parent_dir->inode, io_buf);

    memset(io_buf, 0, 1024);
    //c将新创建文件的inode结点内容同步到硬盘
    inode_sync(cur_part, new_file_inode, io_buf);

    //d将inode_bitmap位图同步到硬盘
    bitmap_sync(cur_part, inode_no, INODE_BITMAP);

    //e将创建的文件inode结点添加到open_inodes链表
...
int32_t file_open(uint32_t inode_no, uint8_t flag) {
    ...
    if(flag & O_WRONLY || flag & O_RDWR) {	//只要是写文件，判断是否有其他进程正在写，读文件不用管
        //以下进入临界区前先关中断
        enum intr_status old_status = intr_disable();
        if(!(*write_deny)) {		//若没有其他进程正在写，占用
            *write_deny = true;		//置为true,避免多个进程同时写此文件
            intr_set_status(old_status);
        }
        else {						//直接返回失败
            intr_set_status(old_status);
            printk("file can't be write now\n");
            return -1;
        }
    }//若是读文件或创建文件，则不用管write_deny
    return pcb_fd_install(fd_idx);
}
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
    // printk("%s\n",buf);
    if((file->fd_inode->i_size + count) > (BLOCK_SIZE * 140)) {     //文件目前最大支持512*140=71680字节
    ...
    uint32_t block_bitmap_idx = 0;  //用来记录把block对应于block_bitmap中的索引，作为参数传给bitmap_sync
    uint32_t sec_idx;				//用来索引扇区
    uint32_t sec_lba;				//扇区地址
    uint32_t sec_off_bytes;			//扇区内的字节偏移量
    uint32_t sec_left_bytes;		//扇区内剩余字节量
    uint32_t chunk_size;			//每次写入硬盘的数据块大小
    int32_t indirect_block_table;	//用来获取一级间接表地址
    uint32_t block_idx;				//用来索引块
    ...
    if(add_blocks == 0) {
        ...
        //若有增量，便涉及到分配新扇区及是否分配一级间接块表，下面分三种情况处理
        //第一种情况：12个直接块够用
        if(file_will_use_blocks <= 12) {
            ...
            while(block_idx < file_will_use_blocks) {
                ...
                //写文件时，不应该在块未使用但已经分配扇区的情况，当文件删除时，就会把地址清0
            ...
            //已使用的间接块也将也将被读入all_blocks,无需单独收录
...
int32_t file_read(struct file* file, void* buf, uint32_t count) {
    ...
    //若要读取的字节数超过了文件可读的剩余量，就用剩余量为待读取的字节数
    if((file->fd_pos + count) > file->fd_inode->i_size) {
        size = file->fd_inode->i_size - file->fd_pos;
        size_left = size;
        if(size == 0) {		//若到文件尾，返回-1
            return -1;
        }
    }
    ...
    uint32_t block_read_start_idx = file->fd_pos / BLOCK_SIZE;			//数据块所在的起始地址
    uint32_t block_read_end_idx = (file->fd_pos + size) / BLOCK_SIZE;   //数据块所在的终止地址
    ...
    if(read_blocks == 0) {			//在同一扇区内读数据，不涉及跨扇区读写
        ...
        if(block_read_end_idx < 12) {
            ...
            ide_read(cur_part->my_disk, indirect_block_table, all_blocks + 12, 1);	//读取一级间接块表
        }
    }

    //用到的地址已经收集到all_blocks中，下面开始读数据
    uint32_t sec_idx;
    uint32_t sec_lba;
    ...
    while(bytes_read < size) {		//直到读完为止
        sec_idx = file->fd_pos / BLOCK_SIZE;		//当前要读的块在all_blocks中的索引
        sec_lba = all_blocks[sec_idx];
        sec_off_bytes = file->fd_pos % BLOCK_SIZE;		//当前要读的块在扇区内的偏移
        sec_left_bytes = BLOCK_SIZE - sec_off_bytes;		//当前要读的块剩余的字节数
        chunk_size = size_left < sec_left_bytes ? size_left : sec_left_bytes;		//本次要读的字节数
