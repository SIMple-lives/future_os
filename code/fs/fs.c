...
static bool mount_partition(struct list_elem* pelem, int arg) {
    ...
    if(!strcmp(part->name, part_name)) {
        ...
        if(cur_part->sb == NULL) {
            PANIC("alloc memsory for partition sb failed");
        }
...
static void partition_format(struct partition* part) {
    ...
    uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR);        //inode结点位图占用的扇区数，最多支持4096个文件
    uint32_t inode_table_sects = DIV_ROUND_UP(((sizeof(struct inode) * MAX_FILES_PER_PART)), SECTOR_SIZE);    //inode表占用的扇区数
    uint32_t used_sects = boot_sector_sects + super_block_sects + inode_bitmap_sects + inode_table_sects;    //已经使用的扇区数
    ...
    sb.sec_cnt = part->sec_cnt;                         //总扇区数
    sb.inode_cnt = MAX_FILES_PER_PART;                  //inode结点数
    sb.part_lba_base = part->start_lba;                 //分区起始扇区

    sb.block_bitmap_lba = sb.part_lba_base + 2;         //块位图起始扇区,0-mbr,1-superblock
    sb.block_bitmap_sects = block_bitmap_sects;         //块位图占用的扇区数

    sb.inode_bitmap_lba = sb.block_bitmap_lba + sb.block_bitmap_sects; //inode位图起始扇区
    sb.inode_bitmap_sects = inode_bitmap_sects;         //inode位图占用的扇区数

    sb.inode_table_lba = sb.inode_bitmap_lba + sb.inode_bitmap_sects; //inode表起始扇区
    sb.inode_table_sects = inode_table_sects;         //inode表占用的扇区数
    ...
    struct disk* hd = part->my_disk; //获得分区所在的主盘
    /*******************************
     * 1 将超级块写入本分区的1扇区 *
     ******************************/
    ide_write(hd, part->start_lba + 1, &sb, 1);		//写入引导扇区后面的扇区
    printk("    Super_Block_LBA:0x%x\n", part->start_lba + 1);

    //找出数据量最大的元信息，用其尺寸做存储缓冲区
    uint32_t buf_size = (sb.block_bitmap_sects >= sb.inode_bitmap_sects ? sb.block_bitmap_sects : sb.inode_bitmap_sects);
    buf_size = (buf_size >= sb.inode_table_sects ? buf_size : sb.inode_table_sects) * SECTOR_SIZE;
    uint8_t* buf = (uint8_t* )sys_malloc(buf_size);	//申请的内存由系统清0后返回

    /**************************************
     * 2 将块位图初始化并写入sb.block_bitmap_lba *
     *************************************/
    //初始化块位图block_bitmap
    buf[0] |= 0x01;			//第0块预留给根目录，位图先占位
    uint32_t block_bitmap_last_byte = block_bitmap_bit_len / 8;
    uint8_t block_bitmap_last_bit  = block_bitmap_bit_len % 8;            //****原先写的是uint32_t
    uint32_t last_size = SECTOR_SIZE - (block_bitmap_last_byte % SECTOR_SIZE);	//last_size是位图所在最后一个扇区中，不足一扇区的其余部分

    //1. 先将位图最后一字节到其所在的扇区结束全设置1,即超出实际块数的部分直接置为已占用
    memset(&buf[block_bitmap_last_byte], 0xff, last_size);

    //2. 再将上一步中覆盖的最后一字节内的有效位重新置0
    uint8_t bit_idx = 0;
    while(bit_idx <= block_bitmap_last_bit) {
        buf[block_bitmap_last_byte] &= ~(1 << bit_idx++);        //原先没有bit_idx++，导致bit_idx始终为0，一直清0第0位
    }
     ...
     ***************************************/
    //写inode_table中的第0项，即根目录所在的inode
    memset(buf, 0, buf_size);		//清空缓冲区
    struct inode* i = (struct inode*)buf;
    i->i_size = sb.dir_entry_size * 2;		//根目录中存放了2个目录项，分别是"."和".."
    i->i_no = 0;				//根目录的inode编号为0
    i->i_sectors[0] = sb.data_start_lba;		//根目录的数据块位于data_start_lba扇区
...
//搜索文件pathname,若找到则返回对应的inode，若找不到则返回-1
static int search_file(const char* pathname, struct path_search_record* searched_record) {
    //若果查找的是根目录，为避免下面无用的查找，直接返回已知根目录信息
    // printk("search_file %s   sasdasdasd\n", pathname);
    if(!strcmp(pathname,"/") || !strcmp(pathname, "/.") || !strcmp(pathname, "/..")) {
        searched_record->parent_dir = &root_dir;
        searched_record->file_type = FT_DIRECTORY;
        searched_record->searched_path[0] = 0; 	//搜索路径置为空
        return 0;
    }
    // printk("             3\n");
    ...
    char *name= (char* )sys_malloc(MAX_FILE_NAME_LEN);
    memset(name, 0, MAX_FILE_NAME_LEN);
    uint32_t parent_inode_no = 0;		//父目录的inode号
    // printk("             3\n");
    sub_path = path_parse(sub_path, name);
    while(name[0]) {		//若第一个字符就是结束符，结束循环
        //记录查找过的路径，但不能超过searched_path的最大长度512字节
...
int32_t sys_open(const char* pathname, uint8_t flags) {
    ...
    if(pathname_depth != path_searched_depth) {        //若没有访问到，则说明中间某个目录不存在
    ...
    // printk("       2\n");
    //若是在最后一个路径上没有找到，并且并不是要创建文件，则直接返回
...
int32_t sys_lseek(int32_t fd, int32_t offset, uint8_t whence) {
    ...
    switch(whence) {
        //SEEK_SET 新的读写位置是相对于文件开头在增加offset个位移量
        case SEEK_SET:
            new_pos = offset;
            break;

        //SEE_CUR 新的读写位置是相对于当前读写位置在增加offset个位移量
        case SEEK_CUR:		//offset可正可负
            new_pos = file_size + offset;
            break;

        //SEEK_END 新的读写位置是相对于文件末尾在增加offset个位移量
        case SEEK_END:		//offset应该为负值
            new_pos = file_size + offset;
    }
...
int32_t sys_mkdir(const char* pathname) {
    ...
    if(inode_no != -1) {        //如果找到了同名目录或文件，失败返回
        ...
        uint32_t pathname_depth = path_depth_cnt((char* )pathname);
        uint32_t path_searched_depth = path_depth_cnt(searched_record.searched_path);
        //先判断是否把pathname的各层目录都访问到了，即是否在某个中间目录就找不到文件了
    ...
    struct dir* parent_dir = searched_record.parent_dir;
    //目录名称后可能会有字符'/',所以最后直接用searched_record.searched_path,无'/'
    char* dirname = strrchr(searched_record.searched_path, '/') + 1;        //函数strrchr
    ...
    uint32_t block_bitmap_idx = 0;            //用于记录block对应与block_bitmap中的索引
    int32_t block_lba = -1;
    //为目录分配一个块，用来写入目录.和..
...
//读取目录dir的1个目录项，常规返回其目录地址，到目录尾时或出错时返回NULL
...
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf) {
    ...
    if(parent_dir_inode->i_sectors[12]) {        //若包含了以及间接块表，读入all_blcoks
...
int32_t sys_chdir(const char* path) {
    ...
    if(inode_no != -1) {
        if(searched_record.file_type == FT_DIRECTORY) {
            ...
            printk("sys_chdirL %s is regular file or other!\n", path);
        }
    }
    dir_close(searched_record.parent_dir);
    return ret;
...
int32_t sys_stat(const char* path, struct stat* buf) {
    ...
    int32_t ret = -1;    //默认返回值
    struct path_search_record searched_record;
    memset(&searched_record, 0, sizeof(struct path_search_record));        //记得初始化或清0,否则栈中信息不知道什么
    int inode_no = search_file(path, &searched_record);
    if(inode_no != -1) {
...
void filesys_init() {
    ...
    while(channel_no < channel_cnt) {
        ...
        while (dev_no < 2)
        {
            if(dev_no == 0) {		//跳过主盘kernel.img
                dev_no++;
                continue;
            }
            struct disk* hd = &channels[channel_no].devices[dev_no];
            struct partition* part = hd->prim_parts;		//遍历主分区
            while(part_idx < 12) {			//四个主分区+8个逻辑分区
    ...
    //将当前分区的根目录打开，并放入到当前进程的fd_table中
